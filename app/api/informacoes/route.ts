...
export async function POST(req: Request) {
  try {
    ...
    const { data, error } = await supabase
      .from("informacoes_canil")
      .insert({ titulo, categoria, conteudo, ativo: true })
      .select()
      .single();
...
export async function PATCH(req: Request) {
  try {
    const body = await req.json();
    const { id, titulo, categoria, conteudo, ativo } = body;

    if (!id) return Response.json({ error: "ID obrigatório" }, { status: 400 });

    const { data, error } = await supabase
      .from("informacoes_canil")
      .update({ titulo, categoria, conteudo, ativo, atualizado_em: new Date().toISOString() })
      .eq("id", id)
      .select()
      .single();
...
export async function DELETE(req: Request) {
  try {
    ...
    const { error } = await supabase.from("informacoes_canil").delete().eq("id", id);
    if (error) return Response.json({ error: error.message }, { status: 500 });
